...
$\text{'3'} + \text{'4'} - \text{'5'} = \text{'34'} - \text{'5'} = 29$입니다.
...
# 해결방안
- 문자열이 아닌 숫자만으로 계산하는 함수와 문자열로 A,B를 계산하고 산수도 실시하는 함수를 작성
- 숫자만 하는 계산은 단순하게 계산 가능
- 문자열로 계산하는 부분은 StringBuilder를 이용하여 작성하도록 구현
