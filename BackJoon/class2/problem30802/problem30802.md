...
# 해결방안
- 첫줄은 N으로 총 참가자이고, 두번째 줄은 사이즈별 신청자의 수, 셋쨰 줄에는 티셔츠와 펜 묶음 수
- 티셔츠는 T 묶음, 펜은 P 묶음으로 이루어진다.
- 티셔츠의 사이즈를 사이즈 배열에 넣어 저장한다.
- 저장한 배열을 순서대로 / t로 나눈 몫을 티셔츠 묶음으로 더하고, 나머지가 0이 아니라면 +1 한다.
- pen 또한 묵음은 /p로, 낱개는 % p 의 개수이다.