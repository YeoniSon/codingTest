...
# 해결방안
- 훼손된 값의 자리가 어디인지 확인한다
- 훼손된 자리의 인덱스는 continue;로 넘어간다.
- 짝수는 가중치 1, 홀수는 가중치 3을 곱해서 더해야한다.
- 이를 통해 값을 모두 더한후
- for문을 통해 0부터 9까지의 수를 돌린다.
- 훼손된 자리의 인덱스가 짝수인 경우 sum에 i값을 더한 후 그 값을 %10 했을때 0일경우 i를 출력한다.
- 홀수인경우는 *3이 되어있기 때문에 sum i * 3을 한 후에 % 10을 했을때 0이 되어야 한다. 
- 이 경우에 값을 출력하도록 한다.